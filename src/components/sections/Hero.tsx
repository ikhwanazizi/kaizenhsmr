...
const Hero = async () => {
  ...
  // 2. Append autoplay parameters
  // We check if the URL already has query params ('?') to determine whether to add '&' or '?'
  // mute=1 is required for autoplay to work in most browsers
  const videoUrl = `${rawUrl}${rawUrl.includes("?") ? "&" : "?"}autoplay=1&mute=1&loop=1`;
